...
std::string decode_domain(const std::vector<uint8_t> &data, size_t &offset)
{
    ...
    while (data[offset] != 0)
    {
        if ((data[offset] & DNS_LABEL_POINTER_MASK) == DNS_LABEL_POINTER_MASK)
        {
            ...
            uint16_t pointer = static_cast<uint16_t>(((data[offset] & 0x3F) << 8) | data[offset + 1]);
            offset = pointer;
            jumped = true;
        }
        else
        {
            ...
            result.append(reinterpret_cast<const char *>(&data[offset]), len);
            offset += len;
            result.push_back('.');
        }
    }
...
void log_info(const std::string &message) { std::cout << current_timestamp() << " INFO: " << message << std::endl; }
void log_error(const std::string &message) { std::cerr << current_timestamp() << " ERROR: " << message << std::endl; }
...
uint16_t read16(const std::vector<uint8_t> &buf, size_t pos) { return read_u16(buf, pos); }

void skip_rr(const std::vector<uint8_t> &buf, size_t &off)
{
    // Skip owner name (labels/pointers) until 0
    while (buf[off] != 0)
    {
